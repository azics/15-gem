...
generatePuzzleItems();
let puzzleItems = document.querySelectorAll(".item-puzzle");

if (isSolvable(puzzleSize)) {
  puzzleInfo.textContent = "Puzzle is solvable";
} else {
  puzzleInfo.textContent =
    "Puzzle is not solvable. Press New Game button to start new puzzle";
}
...
puzzleSizes.addEventListener("click", (e) => {
  ...
  if (size) {
    ...
    frameSize.innerHTML = `Frame size: ${puzzleSize}x${puzzleSize}`;
    generatePuzzleItems();
    puzzleItems = document.querySelectorAll(".item-puzzle");
...
function puzzleNumsArray(length) {
  ...
  let emptySpacePosition = getRandomNumber(length);
  let array = [];
  array.push(randomNumber);
  for (let i = 1; i <= length; ) {
    randomNumber = getRandomNumber(length);
    if (emptySpacePosition === i) {
      array.push("");
      i++;
    } else {
...
function generatePuzzleItems() {
  ...
  for (let i = 0; i < puzzleSize * puzzleSize; i++) {
    ...
    puzzleItem.classList.add("item-puzzle");
...
function isSolvable(puzzleSize) {
  ...
  let emptySpacePosition = puzzleArray.indexOf("");
  let positionFromBottom =
    puzzleSize - Math.floor(emptySpacePosition / puzzleSize);

  console.log("inversions: ", count);
  console.log("emptySpacePosition: ", emptySpacePosition);
...
function canMovePuzzleItem(clickedItemPosition) {
  ...
  let moveSide = "";
  if (
    puzzleItems[clickedItemPosition - 1]?.getAttribute("data-state") ===
      "empty" &&
    +clickedItemPosition % puzzleSize != 0
  ) {
    // console.log('left')
    isMovable = true;
    moveSide = "left";
  ...
  if (
    puzzleItems[clickedItemPosition - puzzleSize]?.getAttribute(
      "data-state"
    ) === "empty"
  ) {
    //console.log('top')
...
puzzle.addEventListener("click", (e) => {
  let puzzleItem = e.target.closest(".item-puzzle");
...
function successMessage() {
  if (isSolved()) {
    ...
    successMessageTime.textContent = `${(minutesLabel.innerHTML = pad(
...
successMessageBtn.addEventListener("click", () => {
  ...
  if (isSolvable(puzzleSize)) {
    ...
    puzzleInfo.textContent = "";
    puzzleInfo.textContent =
      "Please Note: Puzzle is not solvable. Press New Game button to start  new puzzle";
  }
